...
public class DomainWarping : MonoBehaviour
{
    ...
    public float GenerateDomainNoise(int x, int z, NoiseSettings defaultNoiseSettings)
    {
        ...
        return GameNoice.OctavePerlin(x + domainOffset.x, z + domainOffset.y, defaultNoiseSettings);
    }

    public Vector2 GenerateDomainOffset(int x, int z)
    {
        var noiseX = GameNoice.OctavePerlin(x, z, noiseDomainX) * amplitudeX;
        var noiseY = GameNoice.OctavePerlin(x, z, noiseDomainY) * amplitudeY;
